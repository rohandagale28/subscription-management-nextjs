...
export const SubscriptionCard: React.FC<SubCardProps> = ({ title, icon, method, amount, date, id }) => {
    return (
        <Card className="main-card w-full h-28 p-4 flex flex-col justify-between relative">
            <CardTitle className="flex flex-row gap-4 pt-0 w-full">
                ...
                <div className="w-10">
                    <DropDownProfiler id={id} />
                </div>
            </CardTitle>
            <CardContent className="flex flex-row justify-between p-0">
                <div className="flex flex-col">
                    <h3 className="text-sm font-semibold">Total Spend: ${Math.round(amount)}</h3>
                    <h4 className="text-xs">Method: {method}</h4>
                </div>
                <div className="flex flex-col items-end">
                    <h3 className="text-sm font-semibold">${Math.round(amount / 12)} / Month</h3>
